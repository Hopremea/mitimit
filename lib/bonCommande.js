...
/* Écran de chargement : la page se construit en JavaScript (grille du catalogue, totaux), et sur une
   ...
   temps que la grille soit prête. Le voile n'apparaît qu'au bout de 150 ms : sur une connexion
   normale la page est déjà là, et il ne clignote donc pas. */
#chargement{position:fixed;inset:0;z-index:80;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;background:var(--bg);opacity:0;animation:bdc-apparait .25s .15s forwards;}
#chargement.parti{opacity:0!important;pointer-events:none;transition:opacity .3s;}
@keyframes bdc-apparait{to{opacity:1;}}
...
// La grille est construite : le voile s'efface. Il est retiré du document ensuite, pour ne pas
// laisser une animation tourner indéfiniment derrière la page.
(function () {
  var voile = document.getElementById("chargement");
  if (!voile) return;
  voile.classList.add("parti");
  setTimeout(function () { if (voile.parentNode) voile.parentNode.removeChild(voile); }, 400);
})();
