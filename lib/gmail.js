...
export async function createDraft({ to, subject, body, appendSignature = true }) {
  const accessToken = await getAccessToken();
  const fromEmail = process.env.GOOGLE_USER_EMAIL || "";
  const fromDisplay = process.env.GMAIL_FROM_NAME || fromEmail;
  const utf8Subject = `=?UTF-8?B?${Buffer.from(subject, "utf8").toString("base64")}?=`;
...
export async function searchMessagesForAddresses(addresses, opts = {}) {
  ...
  for (const id of ids) {
    ...
    out.push({ id, threadId: m.threadId || "", date, heure, subject: decodeEntities(h("subject") || "(sans objet)"), snippet, body: bodyTxt || snippet, address: matched, direction });
  }
