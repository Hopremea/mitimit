...
//
//   GET  /commande            -> page publique : le formulaire de commande (aucun compte, aucun
//                                accès à MITMIT ; c'est le seul lien que reçoit le client).
//   POST /api/commande        -> dépôt d'une commande par le client (public, sans jeton).
//   GET  /api/commande?list=1 -> commandes reçues et non encore traitées (Clerk obligatoire).
//   POST /api/commande {action:"traite"|"ignore"} -> clôture d'une commande (Clerk obligatoire).
//
...
export default async function handler(req, res) {
  const sb = sbClient();
  if (!sb) { res.status(500).json({ error: "Supabase service role non configuré côté serveur (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY)." }); return; }
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (emp) {
        ...
        const { count } = await sb.from("bons_commande").select("id", { count: "exact", head: true }).eq("ip_hash", emp).gte("created_at", depuis);
...
envoyer.addEventListener("click", function () {
  ...
  fetch("/api/commande", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(corps) })
